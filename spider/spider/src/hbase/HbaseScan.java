...
public class HbaseScan {
	public static void main(String[] args) throws IOException {
		...
		HTable table = (HTable) getTable("users");
		Scan scan = new Scan();
		scan.setMaxVersions();
		// 指定最多返回的Cell数目。用于防止一行中有过多的数据，导致OutofMemory错误。
		scan.setBatch(1000);
		scan.addFamily("address".getBytes());

		// scan.setTimeStamp(NumberUtils.toLong("1370336286283"));
		// scan.setTimeRange(NumberUtils.toLong("1370336286283"),
		...
		// Bytes.toBytes("info"), Bytes.toBytes("id"), CompareOp.EQUAL,
		// Bytes.toBytes("1"));
		...
		for (Result r : rs) {
			for (KeyValue kv : r.raw()) {
				System.out.println(String.format("row:%s, family:%s, qualifier:%s, qualifiervalue:%s, timestamp:%s.", Bytes.toString(kv.getRow()), Bytes.toString(kv.getFamily()),
						Bytes.toString(kv.getQualifier()), Bytes.toString(kv.getValue()), kv.getTimestamp()));
			}
		}
