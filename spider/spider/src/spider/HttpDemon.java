...
public class HttpDemon implements Runnable {
	...
	public HttpDemon(String url) {
		this.url = url;
	}

	public static String fetchHtml(String url) {
		if (url == null || HbaseOperate.isRead(url)) {
		...
		String name = "";
		try {
			html = Jsoup.connect(url).userAgent("Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:28.0) Gecko/20100101 Firefox/28.0")
					.header("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8").header("Accept-Language", "zh-cn")
			...
			FileOutputStream fos = new FileOutputStream(path + File.separator + name + ".html");
	...
	public static Set<String> findHref(String html) {
		...
		while (matcher.find()) {
			...
			if (s.indexOf(".js\"") > -1 || s.indexOf(".css\"") > -1 || s.indexOf(".ico\"") > -1 || s.indexOf(".jpg\"") > -1 || s.indexOf(".png\"") > -1) {
				continue;
			}
			s = s.replaceAll("\"", "");
			hrefs.add(s);
